 ...
 *  设置讨论组名称
 *
 *  @param targitId       讨论组 Id。
 *  @param discussionName 讨论组名称。
 *  @param completion     调用完成的处理。
 *  @param error          调用返回的错误信息。
 */
-(void)setDiscussionName:(NSString*)targitId name:(NSString*)discussionName completion:(void (^)())completion error:(void (^)(RCErrorCode status))error;
 ...
 *  设置讨论组成员邀请权限。
 *
 *  @param targitId   目标 Id。根据不同的 conversationType，可能是聊天 Id、讨论组 Id、群组 Id。
 *  @param isOpen     开放状态，默认开放。
 *  @param completion 调用完成的处理。
 *  @param error      调用返回的错误信息。
 */
-(void)setDiscussionInviteStatus:(NSString*)targitId isOpen:(BOOL)isOpen completion:(void (^)())completion error:(void (^)(RCErrorCode status))error;
